...
def train(model, train_loader, optimizer, device, norm_info, config,
    ...
    for data in train_loader:
        ...
        edge_attr = getattr(data, config["edge_attr_type"]).to(device)
        edge_attr_physics = data.edge_attr.to(device)  # Atributos físicos de las aristas
...
def evaluate(model, data_loader, device, norm_info, config, lambda_physics=0.003, use_physics=False, 
    ...
    with torch.no_grad():  # No necesitamos calcular gradientes para la evaluación
        for data in data_loader:
            ...
            # Máximo error de este grafo (aún en valores normalizados)
            max_error_this_graph = error.max().item()
            
            # Actualizar el máximo global y la suma
    ...
    max_error_overall_K = max_error_overall * norm_info["max_T_real"]
    mean_max_error_per_graph_K = (sum_max_errors_per_graph / num_graphs) * norm_info["max_T_real"]
...
def predict(model, data_loader, device, norm_info, config, plot_results=False):
    """
...
def plot_temperatures_and_error(graph_idx, data, norm_info, T_predicted):
    ...
    G = nx.Graph()
    Showexternal_ids = False  # Cambiar a False si no se quieren usar IDs externos
