...
def evaluate(model, data_loader, device, norm_info, lambda_physics=0.003, use_physics=False, 
    ...
    """
    model.eval()
    error_threshold_K_norm = error_threshold_K / norm_info["max_T_real"]
    ...
    with torch.no_grad():
        for data in data_loader:
            ...
            accuracy = torch.mean((error < error_threshold_K_norm).float()) * 100

            # === NUEVO: Errores máximos ===
        ...
        if plot_results:
            plot_graph_idx = 0
            data = data_loader.dataset[plot_graph_idx].to(device)
            inputs = data.x.to(device)
            targets = data.y.to(device)
            edge_index = data.edge_index.to(device)
